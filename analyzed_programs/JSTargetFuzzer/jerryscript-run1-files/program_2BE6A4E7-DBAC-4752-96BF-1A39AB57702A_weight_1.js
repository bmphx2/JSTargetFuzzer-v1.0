const v1 = new Date();
const v3 = [[Date,Date,v1,Date,Date],v1];
const v4 = [v3];
function F5(a7) {
    if (!new.target) { throw 'must be called with new'; }
    this.g = F5;
}
new F5(v4);
new F5(v4);
new F5(v3);
[-2033630565,8,-1597718440,267641930];
[2147483647,8,1,268435439,2147483648,4294967296,-1024,42441];
[4294967297,128,-524149551,1499780537,28920];
const v18 = [536870889,4294967297,257,-9223372036854775807,360739853,-1995398395,1164864627,-1024,1073741824,-65535];
[2,4,1073741824,4096,-16,-56099,-4294967297,51959,11,2105999656];
[-1024,64,-11051,255,-39832,-33304];
const v23 = new Int32Array(1000);
new Uint16Array(1024);
let v28 = Int32Array;
const v29 = new v28(0);
const v30 = [1,-12];
[-6];
[-9007199254740992];
function f33() {
    return 0;
}
v29[Int32Array];
new Float32Array(3701);
let v43 = new Int32Array();
const v44 = new Int32Array();
function F45(a47) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v44;
    v28 = a47;
    ~this.__proto__;
    let [] = v23;
    this.e = v43;
}
new Int32Array(6);
const v56 = new Float32Array(0);
[-Infinity];
const o60 = {
    set a(a59) {
    },
};
for (let v61 = 0; v61 < 5; v61++) {
    o60.a;
}
const v63 = [-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v65 = [13,9,-12559,-8,-4294967297];
let v66 = [65535,-1139741600];
v56[v18];
let v68;
try { v68 = v18.entries(); } catch (e) {}
new Uint16Array(v68, 1024, v65, v68);
try { v68(); } catch (e) {}
try { v30.flatMap(f33, Uint16Array); } catch (e) {}
new Uint8Array(v65);
let v74 = -41990;
const v75 = v66++;
const v76 = --v74;
function f77() {
    return v28;
}
[-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914];
v63.flatMap(TypeError);
--v43 * (v75 << v76);
