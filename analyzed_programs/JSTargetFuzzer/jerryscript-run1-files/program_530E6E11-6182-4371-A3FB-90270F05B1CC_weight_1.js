const v6 = [-4294967295,-2,-2147483647,-2,-40022,34668,-678880053,-6124];
const v7 = [9223372036854775807,55317,-1882128822,-43546,-4294967295,-11,64,5];
const v8 = [3,-30000,10000,-62040,-9007199254740992];
function F9(a11, a12, a13) {
    if (!new.target) { throw 'must be called with new'; }
    this.g = a12;
    this.e = a11;
}
new F9("symbol", "-256", "-256");
const v15 = new F9("undefined", "symbol", F9);
const v16 = new F9("symbol", "-256", 5401);
const v17 = v8.__proto__;
try { new v17(v15, v16); } catch (e) {}
v7[v15] = v15;
"symbol" && v6;
[v8,509441560];
const v24 = ["symbol"];
[v8,v24,v15,v24,536870887];
const v28 = new Float32Array(3701);
const v31 = new Int32Array();
const v32 = new Int32Array();
function F33(a35) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v32;
    for (const v36 in v8) {
        try { v8.indexOf(v36, 536870887); } catch (e) {}
        v24[Symbol.species] = v36;
    }
    this.e = v31;
}
new Int32Array(6);
new Float32Array(0);
[-Infinity];
const o47 = {
    set a(a46) {
    },
};
for (let v48 = 0; v48 < 5; v48++) {
    const v49 = o47.a;
    if (v49 === Float32Array) {
        try { new v49(); } catch (e) {}
    } else {
        Math.ceil(3701);
        Math.min(3701);
        Math.tan(v48);
        const v57 = -9007199254740990 ^ v48;
        v57 | -9007199254740990;
        Math.exp(v57);
    }
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v62 = [13,9,-12559,-8,-4294967297];
let v63 = [65535,-1139741600];
new Uint8Array(v62);
let v66 = -41990;
const v67 = v63++;
let v68 = --v66;
function f69() {
    return f69;
}
const v70 = [-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914];
v70.flatMap(TypeError);
const v75 = --v68 * (v67 << v66);
o47.a = 5401;
[v75];
["symbol",v28,509441560,5401];
[Int32Array,v70];
