new Int16Array(4096);
const v5 = new Uint32Array(173);
const v8 = new BigInt64Array(3160);
new Float32Array(3701);
const v17 = new Int32Array();
const v18 = new Int32Array();
function F19(a21) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v18;
    try { v8.subarray(a21); } catch (e) {}
    -57380n != -57380n;
    try { undefined.o(undefined, undefined, undefined); } catch (e) {}
    [,,];
    this.e = v17;
    [1225454641,59870,-45491,-3];
    [291386542,57499,7,1073741824,2120456611,12,-1125121794,9];
    [9223372036854775807,8,37099];
    const o35 = {
    };
    new Proxy(v5, o35);
}
new Int32Array(6);
const v41 = new Promise(F19);
const v44 = new Uint32Array(2334);
v44.join(v41);
new Float32Array(0);
[-Infinity];
const o51 = {
    set a(a50) {
    },
};
for (let v52 = 0; v52 < 5; v52++) {
    o51.a;
}
[-6.568975937209441,1.0,-Infinity];
const v57 = new Float32Array(127);
let v58;
try { v58 = F19(v57); } catch (e) {}
const v60 = ([v58]).flat();
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v62 = [13,9,-12559,-8,-4294967297];
let v63 = [65535,-1139741600];
new Uint8Array(v62);
let v66 = -41990;
const v67 = v63++;
let v68 = --v66;
function f69() {
    return f69;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
const v73 = --v68;
const v74 = v67 << v66;
o51.a **= 4096;
v60.length;
v73 * v74;
