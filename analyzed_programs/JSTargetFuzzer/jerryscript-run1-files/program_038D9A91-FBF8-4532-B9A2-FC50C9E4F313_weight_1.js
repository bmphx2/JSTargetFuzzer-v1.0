const v0 = [];
function F4(a6, a7) {
    if (!new.target) { throw 'must be called with new'; }
    this.h = a6;
    this.f = a7;
}
const v8 = new F4(775.0123954576272, 775.0123954576272);
const v9 = new F4(3.8883205770851976, 3.8883205770851976);
new F4(775.0123954576272, 3.8883205770851976);
[-855906021,65536];
const v12 = [-2,-4,16,50324,31923,1024];
[0,536870888,5,-15,1584761111,268435439,9223372036854775807,-44582,2];
function F14() {
    if (!new.target) { throw 'must be called with new'; }
    F14.c = -4294967297;
}
const v17 = new F14();
const v18 = new F14();
new F14();
function f21() {
    function f22(a23, a24, a25) {
        new F14();
        return v17;
    }
    f22(v17, v0, v0);
    const v30 = new Uint32Array(858);
    return v30;
}
function f33() {
    return v17;
}
const v36 = new BigUint64Array(BigUint64Array, BigUint64Array);
v36.copyWithin();
const v40 = RegExp.bind(2);
Math.fround();
Math.min(0.2148301603058278);
let v46 = 1073741824n;
v46 &= v46;
function f49() {
    return v18;
}
v18.__proto__ = v12;
v9.g = v36;
try { Float32Array(); } catch (e) {}
(-12n).toString();
([2036046200,-9223372036854775807,-61646])["map"](Array);
new Float64Array(16);
class C61 {
}
v0.length;
v18.__proto__ = v8;
const v67 = new v40(1);
const v70 = new Int8Array(107);
for (let i73 = 0, i74 = 10; i73 < i74;) {
    v70[v67] = i73;
    ++i73;
    Math.max(-7);
}
new Int16Array(26);
4 == [4,7,4294967297,-1,1000];
const v90 = new Int8Array(Int8Array, Int8Array, Int8Array);
v90.includes(v90, v90);
new Uint8ClampedArray(6);
try {
    new WeakSet(4087n);
} catch(e97) {
} finally {
}
