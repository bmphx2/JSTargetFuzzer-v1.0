const v0 = [-9007199254740991,1412117169,0,53089,1,1,-60775,28633,6,-543769462];
const v1 = [-256,1447071043,4294967296,-12,536870888,512,4096,65535,16029];
const v2 = [536870912,10000,11,5,-14,-634210912,64,-14];
function f3(a4, a5) {
    const o16 = {
        n(a7) {
            v0 in this;
            function F9(a11, a12) {
                if (!new.target) { throw 'must be called with new'; }
                this.c = a11;
            }
            new F9(v0, v0);
            new F9(a7, v1);
            new F9(a5, this);
            return a5;
        },
        "f": v0,
        "c": v0,
        "d": v1,
        "e": v1,
    };
    return o16;
}
f3(v1, v2);
f3(v1, v1);
const v19 = f3(v2, v0);
function f26() {
    return v19;
}
const v27 = [536870889,4294967297,257,-9223372036854775807,360739853,-1995398395,1164864627,-1024,1073741824,-65535];
[2,4,1073741824,4096,-16,-56099,-4294967297,51959,11,2105999656];
[-1024,64,-11051,255,-39832,-33304];
const v32 = new Int32Array(1000);
new Uint16Array(1024);
let v37 = Int32Array;
const v38 = new v37(0);
const v39 = [1,-12];
[-6];
[-9007199254740992];
function f42() {
    return 0;
}
v38[Int32Array];
new Float32Array(3701);
let v52 = new Int32Array();
const v53 = new Int32Array(f26, v52, v52);
function F54(a56) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v53;
    v37 = a56;
    ~this.__proto__;
    let [] = v32;
    this.e = v52;
}
const v62 = new Int32Array(6);
const v65 = new Float32Array(0);
[-Infinity];
const o69 = {
    set a(a68) {
    },
};
for (let v70 = 0; v70 < 5; v70++) {
    o69.a;
}
[-2.220446049250313e-16,3.0,0.0,0.6912577515735743];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v74 = [13,9,-12559,-8,-4294967297];
let v75 = [65535,-1139741600];
v65[v27];
let v77;
try { v77 = v27.entries(); } catch (e) {}
new Uint16Array(v77, 1024, v74, v77);
try { v77(); } catch (e) {}
try { v39.flatMap(f42, Uint16Array); } catch (e) {}
new Uint8Array(v74);
let v83 = -41990;
const v84 = v75++;
const v85 = --v83;
function f86() {
    return v62;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v52 * (v84 << v85);
