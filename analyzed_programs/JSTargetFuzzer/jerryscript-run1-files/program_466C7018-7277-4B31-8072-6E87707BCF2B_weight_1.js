const v0 = [0.21170948966267433,917.2340314554458,-2.2250738585072014e-308,83845.5045483713,-3.3821838543660236e+307,-1.0,7.502903278324055,60839.366496085655];
const v1 = [1e-15,552005.8414225681,444238.05778522696,1.5646126965811114,2.220446049250313e-16,-947215.8485820505,7.657373406317941,-2.0,NaN,-831.6897702754434];
const v2 = [1e-15,924659.8056199152];
function F3(a5, a6) {
    if (!new.target) { throw 'must be called with new'; }
    this.c = a5;
}
const v7 = new F3(v1, v1);
const v8 = new F3(v2, v2);
let v9 = new F3(v2, v1);
class C10 extends F3 {
    [v2];
    constructor(a12, a13, a14) {
        super(a12, a12);
        const v15 = a14.c;
        v9 - v1;
        try { v1.some(v15, v7); } catch (e) {}
    }
    get h() {
        new Map();
        v7[v9];
        return "Uyt";
    }
}
let v25 = new C10(v0, v7, v9);
v25 %= v7;
F3 |= C10;
v7.c;
const v29 = new Uint16Array(34);
new Uint16Array(36);
new Int32Array(3927);
new C10(v0, v8, v7);
new C10(v0, v9, v8);
const v45 = new Map();
function f47() {
    return f47;
}
v9 &= f47;
for (let v48 = 0; v48 < 25; v48++) {
    function f54(a55, a56, a57) {
        const o67 = {
            [a56]: "p",
            "a": a56,
            ...v29,
            "d": 0,
            "b": 3927,
            n(a59, a60, a61, a62) {
                a59[a57];
                [257,9223372036854775807,-268435456,10,2,-9223372036854775807,-4294967297,1073741825,1073741825,1000];
                [609089358,-1,-2147483648,10,1073741823,4294967296,65535,2480,65537,-128];
                [1024,65535,1024,16,4,49676,-2147483648,-5,57864,-10355];
                return a56;
            },
            "a": 6,
            ...a55,
            ..."description",
            ..."__proto__",
            "g": a56,
            "e": "__proto__",
            [a55]: a56,
            "c": -27883,
        };
        return o67;
    }
    const t64 = 523;
    t64["p" + v48] = v48;
}
const v70 = new Uint16Array(523);
new Uint8ClampedArray(197);
new Float32Array(5);
v45.size;
new Uint8Array(8);
const v86 = new Uint8ClampedArray(1024);
let v89 = new Uint16Array(239);
let v91 = 44256;
new Uint8ClampedArray(255);
const v98 = new Uint32Array(858);
const v101 = new Uint16Array(7);
function f102(a103, a104, a105, a106) {
    for (let i109 = 0, i110 = 10; i109 < i110; i109++, i110--) {
        [0.342278422626182];
        [0.8942963082519859,-904364.6886423089,-909.5766504631073,1000000000.0,2.2250738585072014e-308,-21219.590454767807,0.32217532431361673,2.0];
        [1.0,0.1323375954903766];
    }
    const o160 = {
        m(a122, a123, a124) {
            v91 ^= v0;
            [-5.0,0.6176320614789462,2.4796066441118434,1.8010887102075852,-1.0,-4.0,-3.0,1.0];
            [-2.660624983254484e+307,4.0,-6.224343469236721e+307,-1.7976931348623157e+308,-7.781808913468946e+307,-569211.3075469795,-316.07372880244884,-847553.4946799515];
            [169.64171117192063,-609574.235028561,-9.613757066389756,758172.407755943,1.2851649532322922e+308,-7.705740903077952e+306,5.0,323000.4456514192,-575157.0949599417];
            function F128(a130, a131, a132, a133) {
                if (!new.target) { throw 'must be called with new'; }
                a131 === -4294967295;
            }
            let v138 = -1489112994;
            if ("o") {
                v138 += -65537;
            }
            super.e = Uint16Array;
            super.e = v138;
            typeof 255 === "bigint";
            [1355297802,6,6,2147483648,-37359,-1105049582,65535,-256,6317];
            [4];
            [14,-4294967295,64,6,1000];
            v89 = v70;
            this instanceof Uint16Array;
            return Uint16Array;
        },
        set f(a150) {
            v101[this] = a105;
            let v151;
            try { v151 = this.m(-4096, 0); } catch (e) {}
            ({"a":v151,} = this);
            [257,536870888];
            [65536,9,1724,37005,-65535,-14,-14,2,4096,5];
            ("reverse").matchAll(Date.__proto__);
            this();
            [2147483647,1163760765,58060,37249,470747050,9007199254740991,-1019824430,1,-268435456,256];
            return a150;
        },
    };
    return o160;
}
const v161 = f102(858, 255, 255, v91);
const v162 = f102(-4096, 255, 858, v101);
function F163(a165, a166, a167) {
    if (!new.target) { throw 'must be called with new'; }
    this.b = v86;
}
const v168 = [9007199254740990];
const v169 = [3,-442752321,-1397339341,4294967296];
function f170() {
    v169.e = -4096;
    "p" + ("toString").trimLeft;
    return v0;
}
function f176(a177) {
    const o197 = {
        set e(a179) {
            function F180(a182, a183, a184) {
                if (!new.target) { throw 'must be called with new'; }
                this.c = a184;
            }
            Math.expm1(Math);
            const v188 = new Float32Array(Float32Array, Float32Array);
            v188 * 34;
            const v190 = [-11,-28972,65535,12,536870912,-2,536870889,50897,8];
            for (let v191 = 0; v191 < 10; v191++) {
                v190.includes(v191);
            }
            try { this.lastIndexOf(f170, a177, a177, a179, this); } catch (e) {}
        },
        __proto__: a177,
        [a177]: a177,
        "g": v168,
        [v0]: v169,
        ...v169,
        [v169]: v0,
        "a": v0,
        ...v169,
        [f170]: v169,
    };
    return o197;
}
const o198 = {
};
const v201 = [[null,null],null];
const o203 = {
    [v201]: -6n,
};
f176(v0);
f176(v168);
const o206 = {
    __proto__: v168,
};
f102(255, 255, -4096, 255);
v161[8] |= 7;
for (const v208 in v98) {
    const v210 = [v208,f102,255];
    Reflect.apply(v162.m, v101, v210);
    function f213() {
        const v215 = new WeakMap();
        const v217 = new Uint32Array();
        v217 >= v215;
        return v208;
    }
}
