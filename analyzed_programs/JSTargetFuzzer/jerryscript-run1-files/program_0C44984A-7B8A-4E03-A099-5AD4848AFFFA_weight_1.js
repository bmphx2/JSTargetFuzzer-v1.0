function f0() {
}
let v1 = 7;
let v5 = 88.87389665792466;
v5.__proto__ = v1;
let v7 = Date;
const v8 = new v7();
({"a":v1,"d":v5,"e":v7,} = v8);
typeof 1073741824 === "boolean";
function F12(a14, a15, a16) {
    if (!new.target) { throw 'must be called with new'; }
    this.c = a15;
}
const v17 = new F12(NaN, v5, F12);
new F12(-858132.5841251275, NaN, F12);
const v19 = new F12(-858132.5841251275, NaN, 1073741824);
const v20 = [1000];
[v19,1073741824];
const v22 = [v19,v20,v19,1073741824,v17];
function F23() {
    if (!new.target) { throw 'must be called with new'; }
    F23.c = -4294967297;
}
const v26 = new F23();
new F23();
new F23();
function f30() {
    const v33 = new Uint32Array(858);
    return v33;
}
for (let v36 of v22) {
    v36 = F23;
}
function f40() {
    return v26;
}
const v43 = new BigUint64Array(BigUint64Array, BigUint64Array);
v43.copyWithin();
let v47 = -12n;
v47 >>= v47;
const v48 = RegExp.bind(2);
Math.fround();
Math.min(0.2148301603058278);
let v54 = 1073741824n;
v54 &= v54;
try { Float32Array(); } catch (e) {}
(-12n).toString();
([2036046200,-9223372036854775807,-61646])["map"](Array);
new Float64Array(16);
class C68 {
}
const v70 = new v48(1);
const v73 = new Int8Array(107);
for (let i76 = 0, i77 = 10; i76 < i77;) {
    v73[v70] = i76;
    ++i76;
    Math.max(-7);
}
new Int16Array(26);
4 == [4,7,4294967297,-1,1000];
const v93 = new Int8Array(Int8Array, Int8Array, Int8Array);
v93.includes(v93, v93);
new Uint8ClampedArray(6);
try {
    new WeakSet(4087n);
} catch(e100) {
} finally {
}
