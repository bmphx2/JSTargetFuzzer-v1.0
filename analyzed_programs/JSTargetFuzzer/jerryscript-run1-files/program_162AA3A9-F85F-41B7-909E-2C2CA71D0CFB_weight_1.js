function f0() {
}
const v2 = new Set();
v2.prototype ^= 0.9875862750480174;
const v7 = /d/i * /(ab|cde)+/gyu;
const v8 = [5,-60561,10,65535,-3];
const v9 = [-38166,0,-11,-3,257];
RangeError();
v8.sort();
const v17 = [[[4,v7,v7],-4096],v7];
try { (0.9875862750480174).values(); } catch (e) {}
const v19 = [0];
("2118791066").trimRight();
Object.defineProperty([-9007199254740991,54726,1717830925,31978,16,4294967297,-28128,-15,-806491297,127], 256, { writable: true, configurable: true, enumerable: true, value: [0.6476119767844933,-2.0] });
let v28 = -4028n;
v28 += f0;
function F29(a31, a32, a33) {
    if (!new.target) { throw 'must be called with new'; }
    this.c = a33;
    if (a32) {
        v9[v9] = 4;
    } else {
        v17 instanceof a33;
    }
}
const v37 = [65537,-345187051];
const v38 = [v37,v37,v19,v37,v37];
const v39 = [v38,v38,v38];
class C40 {
    constructor(a42, a43, a44) {
        try { arguments.n(); } catch (e) {}
    }
    /*
    constructor(a48, a49, a50, a51) {
        v38.unshift(v37);
        for (let v53 = 0; v53 < 32; v53++) {
            v39["p" + v53] = v53;
        }
        const v57 = Symbol.replace;
        let v58 = -2147483648n;
        v58 &= 1546754096n;
        for (const v61 in Math) {
        }
        v38[v57] = v39;
    }
    */
}
const v62 = new C40();
new C40(v62, v37, v37, v37);
