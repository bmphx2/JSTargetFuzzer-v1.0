class C3 {
    constructor(a5) {
        (a5 >>>= this) || "setUTCSeconds";
    }
    static valueOf(a8) {
        try { a8["n"]("n", 1000.0); } catch (e) {}
        return "n";
    }
    static [64] = 1000.0;
}
new C3(C3);
new C3("setUTCSeconds");
new C3(C3);
function f14() {
    return C3;
}
new Float32Array(3701);
const v23 = new RangeError();
const v24 = new RangeError();
new ReferenceError(ReferenceError);
function F27(a29) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v24;
    this.e = v23;
}
new RangeError(6);
new Float32Array(0);
[-Infinity];
const o37 = {
    set a(a36) {
    },
};
for (let v38 = 0; v38 < 5; v38++) {
    o37.PI;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v42 = [10000,-1013851052,1073741825,-1683859626];
let v43 = [65535,-1139741600];
new Uint8ClampedArray(v42);
let v46 = -41990;
const v47 = v43++;
let v48 = --v46;
function f49() {
    return f49;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v48 * (v47 << v46);
