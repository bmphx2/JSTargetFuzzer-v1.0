const v3 = [11,-1532993960,-9,15,-1103];
const v4 = [-4294967297,-8,127,-1,129,988180180];
const v5 = [-4294967296,9007199254740992,15,127,1024,-65537,219928530,2,14684];
new Float32Array(3701);
const v17 = new Int32Array();
const v18 = new Int32Array();
function F19(a21) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v18;
    this.e = v17;
}
const v22 = new Int32Array(6);
v17.valueOf = Int32Array;
v22[-1073741824] = 10;
new Float32Array(0);
[-Infinity];
const o67 = {
    set a(a31) {
    },
    n(a33, a34) {
        new Uint32Array(1599);
        delete v4[10];
        const o42 = {
            "maxByteLength": 86,
        };
        const v44 = new ArrayBuffer(86, o42);
        new Uint8ClampedArray(v44);
        v5[v3] &= 65536;
        const o50 = {
            "maxByteLength": 536870889,
        };
        const v52 = new ArrayBuffer(8, o50);
        new Float32Array(v52);
        const v56 = new Float32Array(1);
        this * v56;
        new Uint32Array(16);
        return Float32Array;
    },
};
for (let v68 = 0; v68 < 5; v68++) {
    o67.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v72 = [13,9,-12559,-8,-4294967297];
let v73 = [65535,-1139741600];
new Uint8Array(v72);
let v76 = -41990;
const v77 = v73++;
let v78 = --v76;
function f79() {
    return f79;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v78 * (v77 << v76);
