const v0 = [];
const v4 = [35936,65536,64];
const v5 = [268435441,1073741824,2147483647,4,-57283,-36951];
[2147483648,976820559,-37145,7,-14,14,16,-13,2147483648,2147483649];
const v7 = [10337,-45328,9007199254740991,-4096,27839,268435439,63804,-1967169789,-268435456];
[1073741823,0,3,754351377];
const v9 = [2,9,1816576397,2147483649,654859751,-22721,40737,1889];
class C10 {
    static get h() {
        try { v7(this, v4, this); } catch (e) {}
        const v14 = [];
        Reflect.apply(v9.values, v0, v14);
        Math.sin(7.453223156928054);
        return Math.trunc(~0);
    }
}
const v24 = new C10();
new C10();
new C10();
const v29 = new Float32Array(3701);
const v32 = new Int32Array();
const v33 = new Int32Array();
function F34(a36) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v33;
    this.e = v32;
}
new Int32Array(6);
const v40 = new Float32Array(0);
[-Infinity];
const o44 = {
    set a(a43) {
    },
};
for (let v45 = 0; v45 < 5; v45++) {
    o44.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
v33[v5] = v24 >= v29 ? v24 : v29;
v40[6];
const v55 = [13,9,-12559,-8,-4294967297];
let v56 = [65535,-1139741600];
new Uint8Array(v55);
let v59 = -41990;
const v60 = v56++;
let v61 = --v59;
function f62() {
    return f62;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v61 * (v60 << v59);
