const v2 = new Uint16Array(402);
new BigInt64Array(6);
new Uint8ClampedArray(472);
function f9() {
    return v2;
}
new Date();
function F24() {
    if (!new.target) { throw 'must be called with new'; }
    F24.prototype = 1433387171;
}
const v27 = new F24();
const v28 = new F24();
new F24();
function f31() {
    const v34 = new Uint32Array(858);
    return v34;
}
f31();
function f38() {
    return f38;
}
function f39() {
    return f39;
}
try {
    Object.defineProperty(v28, 2, { configurable: true, enumerable: true, get: f38, set: f39 });
} catch(e40) {
} finally {
}
function f41() {
    return v27;
}
const v44 = new BigUint64Array(BigUint64Array, BigUint64Array);
v44.copyWithin();
const v48 = RegExp.bind(2);
Math.fround();
let v53 = Math.min(NaN);
const v58 = Array["of"](0.0, 203);
let v59 = 1073741824n;
v59 &= v59;
function f62(a63, a64) {
    const o65 = {
    };
    return o65;
}
const v66 = f62();
for (let i = 0; i < 5; i++) {
    Object.defineProperty(v66, 32753n, { enumerable: true, get: f62 });
}
try { Float32Array(); } catch (e) {}
(-12n).toString();
const v71 = [1000000.0,1.2694373884119616e+308,-Infinity,0.7652992022490531,5.0,-8.644737118571141,1000000.0,0.9075298547528431,-2.220446049250313e-16];
v71.length <<= -62483;
v71.reverse();
try {
    new Date(4087n);
} catch(e77) {
} finally {
}
([2036046200,-9223372036854775807,-61646])["map"](Array);
new Float64Array(v58);
class C86 {
}
const v88 = new v48(1);
const v91 = new Int8Array(107);
for (let i94 = 0, i95 = 10; i94 < i95;) {
    v91[v88] = i94;
    ++i94;
    Math.max(7);
}
function f106() {
    return f106;
}
v53++;
const o108 = {
};
new Int16Array(26);
4 == [4,7,4294967297,-1,1000];
const v114 = new Int8Array(Int8Array, Int8Array, Int8Array);
v114.includes(v114, v114);
new Uint8ClampedArray(6);
try {
    new WeakSet(-13n);
} catch(e121) {
} finally {
}
