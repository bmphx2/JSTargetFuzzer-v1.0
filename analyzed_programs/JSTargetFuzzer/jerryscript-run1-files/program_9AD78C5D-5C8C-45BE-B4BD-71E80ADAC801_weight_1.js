function F3(a5, a6) {
    if (!new.target) { throw 'must be called with new'; }
    try {
    } catch(e7) {
    } finally {
    }
    [46268,595213550,this,this];
    this.f = 255;
    this.e = 255;
    this.d = a6;
}
new F3(536870912, 536870912);
new F3(1073741825, 255);
new F3(1073741825, 536870912);
const v21 = [-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914];
v21.flatMap(parseInt);
function F24(a26) {
    if (!new.target) { throw 'must be called with new'; }
}
const v28 = new Float32Array(3701);
const v31 = new Int32Array();
const v32 = new Int32Array();
let v33 = "1000";
("b").split(v33);
function F36(a38) {
    if (!new.target) { throw 'must be called with new'; }
    new RegExp(1082);
    class C43 {
        static m(a45) {
            9191n | [v21,this];
        }
    }
    this.f = v32;
    this.e = v31;
}
new Int32Array(6);
class C49 {
    static n(a51) {
        C49 *= -1478895337n;
    }
}
("2147483648").replaceAll("-4096");
JSON.parse(255);
new Float32Array(0);
[-Infinity];
const o65 = {
    set a(a64) {
    },
    ...v28,
};
for (let v66 = 0; v66 < 5; v66++) {
    o65.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v70 = [13,9,-12559,-8,-4294967297];
let v71 = [65535,-1139741600];
new Uint8Array(v70);
const v75 = v71++;
let v76 = --v33;
function f77() {
    return f77;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v76 * (v75 << -41990);
const o84 = {
};
new Uint8ClampedArray();
const v87 = [1.7976931348623157e+308];
const v88 = [-1000.0,-Infinity,-3.0];
function* f89(a90, a91) {
    yield* v87;
    return a91;
}
f89(v87, v88);
