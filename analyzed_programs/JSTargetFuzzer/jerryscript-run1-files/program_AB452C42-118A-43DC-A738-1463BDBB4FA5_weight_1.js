function f0() {
    const o17 = {
        __proto__: "constructor",
        "f": 2,
        "e": 2,
        [2]: 2,
        get g() {
            return this.__proto__;
        },
        set h(a8) {
            let v7 = this;
            const v11 = -217429869;
            let v12 = v11 ** v11;
            ++v12;
            v7--;
            v11 && 2;
            v12++;
        },
    };
    return o17;
}
f0();
f0();
f0();
[-1024,-61317,65536];
const v25 = [-1,268435456,13255];
[2147483647,15,1073741823,15,4096,128,268435456,-9007199254740992,1024];
new WeakMap();
function F32() {
    if (!new.target) { throw 'must be called with new'; }
    F32.c = -4294967297;
}
new F32();
new F32();
new F32();
function f39() {
    const v42 = new Uint32Array(858);
    return v42;
}
function f45() {
    new Uint32Array(858);
    const v50 = new Date();
    return v50;
}
const v53 = new BigUint64Array(BigUint64Array, BigUint64Array);
v53.copyWithin();
const v57 = RegExp.bind(2);
Math.fround();
Math.min(0.2148301603058278);
let v63 = 1073741824n;
v63 &= v63;
try { Float32Array(); } catch (e) {}
v25.toString();
([2036046200,-9223372036854775807,-61646])["map"](Array);
new Float64Array(16);
class C77 {
}
const v79 = new v57(1);
const v82 = new Int8Array(107);
for (let [i90, i91] = (() => {
        const t0 = [256,-14,536870912,1073741824,-9007199254740990,536870889,9,4294967296];
        t0["p" + 2] = 2;
        function f86() {
            return f86;
        }
        [f86,f86,f86];
        return [0, 10];
    })();
    i90 < i91;
    ) {
    v82[v79] = i90;
    ++i90;
    Math.max(-7);
}
async function f101(a102, a103) {
    class C104 extends a103 {
    }
    return C104;
}
new Int16Array(26);
const v108 = [4,7,4294967297,-1,1000];
let v109 = 0n;
v109 /= 22230n;
4 == v108;
const v113 = new Int8Array(Int8Array, Int8Array, Int8Array);
v113.includes(v113, v113);
new Uint8ClampedArray(6);
try {
    new WeakSet(4087n);
} catch(e120) {
} finally {
}
