const v0 = [268435456,489871206,-9007199254740992,268435439,0,-2,55778];
const v1 = [-128,-441933661,54222,5];
const v2 = [-2147483649,-22866];
function f6(a7, a8) {
    const o13 = {
        [v1]: a8,
        0: a7,
        "b": v1,
        "c": v2,
        "h": 10000n,
        set g(a10) {
            a8 += this;
            try { a7.keys(); } catch (e) {}
            delete a7[2700171822];
            super.g = -9223372036854775807n;
        },
        "d": v0,
    };
    return o13;
}
f6(v2, v1);
f6(v0, 10000n);
f6(v2, v0);
new Int8Array(168);
new Int32Array(9);
new Int16Array(2);
new Set();
new WeakMap();
new WeakSet();
const v55 = new URIError(512);
const v58 = new Int16Array(447);
new Uint32Array(2438);
function f65() {
}
v55.description <<= 2006448021;
const v73 = [2.220446049250313e-16,-1.7976931348623157e+308,-3.4833326659982795,-1000000000000.0,0.0,2.220446049250313e-16,-4.191721618721404,1.4104645033139904e+308];
("1073741824").match(v73.copyWithin(v73, v73));
new Uint8Array(0);
const v83 = [-8,4294967295,148900462];
let v84 = RegExp.bind("2147483647", v83);
const v86 = new Uint8Array(127);
function f87(a88, a89) {
    const o90 = {
        "f": a88,
        ...v84,
        "e": a89,
    };
    return o90;
}
f87(v84, 26357);
f87(v83, 26357);
f87(v73, 2);
v84 /= v86;
new WeakSet();
new BigInt64Array(2, -14, v58);
([5]).reverse();
