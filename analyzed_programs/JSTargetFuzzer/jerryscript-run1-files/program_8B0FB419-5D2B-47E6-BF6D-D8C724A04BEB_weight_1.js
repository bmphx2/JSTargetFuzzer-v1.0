function F6(a8, a9, a10, a11) {
    if (!new.target) { throw 'must be called with new'; }
    this.h = "flags";
    function F12(a14, a15) {
        if (!new.target) { throw 'must be called with new'; }
        this.d = "MIN_SAFE_INTEGER";
    }
    const v16 = new F12(a8, a10);
    new F12(v16, "flags");
    new F12(F6, "MIN_SAFE_INTEGER");
    this.c = a8;
    this.g = a8;
}
const t13 = "MIN_SAFE_INTEGER";
t13.toString = ("flags")["concat"]("concat", "flags", "concat");
new BigUint64Array(14);
new Int8Array(7);
const v29 = new Int8Array(1024);
new F6(129, 1073741824, 129, 1073741824);
new F6(88991296, 88991296, 88991296, 1073741824);
new F6(129, 88991296, 88991296, 129);
new Float32Array(3701);
Int8Array << -1008544500;
~v29;
Math.round(v29);
Math.min(v29);
const v48 = new Int32Array();
const v49 = new Int32Array();
function F50(a52) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v49;
    this.e = v48;
}
new Int32Array(6);
new Float32Array(0);
[-Infinity];
const o60 = {
    set a(a59) {
    },
};
for (let v61 = 0; v61 < 5; v61++) {
    o60.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v65 = [13,9,-12559,-8,-4294967297];
let v66 = [65535,-1139741600];
new Uint8Array(v65);
let v69 = -41990;
const v70 = v66++;
let v71 = --v69;
function f72() {
    return f72;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v71 * (v70 << v69);
