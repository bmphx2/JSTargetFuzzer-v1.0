const v2 = new BigUint64Array(5);
new BigInt64Array(3324);
const v8 = new Int16Array(455);
let v11 = new Float64Array(77);
new Uint8Array(1000);
new Float64Array(256);
function F18() {
    if (!new.target) { throw 'must be called with new'; }
    F18.c = -4294967297;
}
const v21 = new F18();
const v22 = new F18();
new F18();
function f25() {
    const v28 = new Uint32Array(858);
    return v28;
}
function f31() {
    return v21;
}
const v34 = new BigUint64Array(BigUint64Array, BigUint64Array);
const v35 = v34.copyWithin();
v2 !== BigInt64Array;
if (v8) {
} else {
    new Int16Array(4096);
    new Uint8Array(29);
    new Uint16Array(255);
}
const v51 = RegExp.bind(2);
Math.fround();
Math.min(0.2148301603058278);
let v57 = 1073741824n;
v57 &= v57;
let v60;
try { v60 = Float32Array(); } catch (e) {}
(-12n).toString();
v60 = v35;
v11 |= v22;
!1000;
f25();
const v66 = v22.e;
const v67 = [2036046200,-9223372036854775807,-61646];
v67["map"](Array);
new Float64Array(16);
class C74 {
}
const v76 = new v51(1);
const v79 = new Int8Array(107);
for (let i82 = 0, i83 = 10; i82 < i83;) {
    v79[v76] = i83;
    ++i82;
    Math.max(-7);
}
new Int16Array(26);
4 == [4,7,4294967297,-1,1000];
const v99 = new Int8Array(Int8Array, Int8Array, Int8Array);
v99.includes(v99, v99);
new Uint8ClampedArray(6);
RegExp(v66);
try { C74.reduce(107, Math, Math, f31); } catch (e) {}
function F106(a108, a109, a110) {
    if (!new.target) { throw 'must be called with new'; }
    this.c = F18;
    this.g = f25;
}
const v111 = new F106(Int16Array, RegExp, v67);
new F106(BigInt64Array, v111, 9007199254740990);
new F106(107, Float64Array, v51);
try {
    new WeakSet(4087n);
} catch(e116) {
} finally {
}
