new BigInt64Array(2917);
new BigInt64Array(252);
new Float32Array(3);
new Map();
function F11() {
    if (!new.target) { throw 'must be called with new'; }
    F11.c = -4294967297;
}
const v14 = new F11();
new F11();
new F11();
const v19 = resourceName.bind(resourceName, resourceName, resourceName);
v19(v19);
new WeakMap();
function f23() {
    const v26 = new Uint32Array(858);
    return v26;
}
function f29() {
    return v14;
}
function f31() {
}
function F32(a34, a35) {
    if (!new.target) { throw 'must be called with new'; }
}
new Int32Array(RegExp.bind("2147483647", [-8,4294967295,148900462]));
const v43 = new BigUint64Array(BigUint64Array, BigUint64Array);
v43.copyWithin();
eval(eval);
RegExp.bind(2);
const v52 = RegExp.bind(2);
Math.fround();
Math.min(0.2148301603058278);
let v58 = 1073741824n;
v58 &= v58;
try { Float32Array(); } catch (e) {}
(-12n).toString();
([2036046200,-9223372036854775807,-61646])["map"](Array);
new Float64Array(16);
-34557n == -9223372036854775808;
const v75 = [-Infinity];
function f78(a79, a80, a81, a82) {
    'use strict';
    delete a81[96111267];
    return a81;
}
f78(257, 7, v75);
new Int32Array(127);
class C88 {
}
const v90 = new v52(1);
const v93 = new Int8Array(107);
for (let i96 = 0, i97 = 10;
    i96 < i97;
    (() => {
        const v102 = new Uint8Array();
        delete v102[9007199254740990];
        let v104 = undefined;
        const o107 = {
            get c() {
                return v104 = arguments;
            },
        };
        const o108 = {
            ...o107,
        };
    })()) {
    v93[v90] = i96;
    ++i96;
    Math.max(-7);
}
new Int16Array(26);
2.220446049250313e-16 - 11;
class C123 {
    constructor(a125, a126) {
    }
    268435440;
    f = 0.7127021906311133;
}
const v127 = new C123(0.2562247708610226, C123);
v127.toString = [C123,C123,C123,C123,C123];
const v131 = new Int32Array(6);
new ArrayBuffer(v131);
4 == [4,7,4294967297,-1,1000];
const v137 = new Int8Array(Int8Array, Int8Array, Int8Array);
v137.includes(v137, v137);
new Uint8ClampedArray(6);
("p").lastIndexOf();
const v144 = [-65537,-1201537790,9,-3,16,-383550289,536870887,-9223372036854775808,-543695968];
for (let v145 = 0; v145 < 5; v145++) {
    Object.defineProperty(v144, "length", { value: v145 });
}
try {
    new WeakSet(4087n);
} catch(e148) {
} finally {
}
