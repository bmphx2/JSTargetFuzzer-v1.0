[1,10,34865,4294967295,-10,257,1000,-9223372036854775808,9054,10];
[-1877958359,10,1298707085,127,65536,-21704,65535,65535,0,-2];
[65537,0,1,63540,-11];
[5,2147483648,-18302];
const v8 = [13];
const v9 = [4,256,-55249,32936];
function f10() {
    return v8;
}
new Float32Array(3701);
const v19 = new Int32Array();
const v20 = new Int32Array();
function F21(a23) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v20;
    this.e = v19;
}
new Int32Array(6);
new Float32Array(0);
const v28 = [-Infinity];
const o33 = {
    set a(a30) {
        function f31() {
            return "Fpj";
        }
        ("Fpj").length;
        Object.defineProperty(this, "d", { writable: true, enumerable: true, set: f31 });
    },
    __proto__: v9,
    "e": Float32Array,
    "c": v9,
    1200: Int32Array,
    ...v28,
};
for (let v34 = 0; v34 < 5; v34++) {
    o33.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v38 = [13,9,-12559,-8,-4294967297];
let v39 = [65535,-1139741600];
if (F21 >= v39) {
    v9 - v39;
    v9.valueOf = f10;
} else {
}
new Uint8Array(v38);
let v45 = -41990;
const v46 = v39++;
let v47 = --v45;
function f48() {
    return f48;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v47 * (v46 << v45);
