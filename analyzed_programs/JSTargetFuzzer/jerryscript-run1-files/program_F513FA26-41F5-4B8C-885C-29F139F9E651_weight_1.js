const v5 = /r(?:a?)?8t(x)(x)(x)\1E(?!(a))\1+/gu;
const v6 = /\ud808\udf45*/gyiu;
v6 > v6;
[-51534,-4294967297,9,-1047957330,1000];
[-256,60780,3,-65535];
[6,-536870912,9];
new Float32Array(3701);
const v23 = new Int32Array();
const v24 = new Int32Array();
function F25(a27) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v24;
    this.e = v23;
}
new Int32Array(6);
const v31 = new Float32Array(0);
[-Infinity];
const o35 = {
    set a(a34) {
    },
};
for (let v36 = 0; v36 < 5; v36++) {
    o35.a;
}
let v38 = [-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v40 = [13,9,-12559,-8,-4294967297];
let v41 = [65535,-1139741600];
new Uint8Array(v40);
let v44 = -41990;
const v45 = v41++;
let v46 = --v44;
function f47() {
    return f47;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
const v51 = --v46;
new Uint32Array(2334);
function f55() {
    v5[6];
    new TypeError();
    ++v38;
    const v61 = !v38;
    (v38 << v31) + v38;
    let v64 = v38 / v61;
    v64--;
    return f55;
}
class C66 extends f55 {
    constructor(a68, a69) {
        super();
        try { C66(this, f55, this, a68); } catch (e) {}
    }
}
new C66(C66, C66);
v51 * (v45 << v44);
