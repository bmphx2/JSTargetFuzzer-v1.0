new BigInt64Array(7);
const v5 = new Uint32Array(3);
new Float64Array(1707);
const v11 = new BigInt64Array(128);
const v14 = new Uint8ClampedArray(2);
new Int16Array(11);
const v21 = [65535n,0n,964329252n];
let v22 = [964329252n,964329252n,v21,v21];
const v23 = [v22,v22,v21,964329252n,0n];
const v24 = [65535n,v22];
const v25 = [v23,0n];
const v26 = [v22];
function f27(a28) {
    return BigInt64Array;
}
class C29 extends f27 {
    static 111 = f27;
    [v25] = Uint32Array;
}
const t19 = "fdNr";
t19[0] = 65535n;
let v36;
try {
const t0 = "localeCompare";
v36 = new t0(v21, v26, v26, 58270);
} catch (e) {}
async function* f37(a38, a39, a40) {
    ({"length":a38,...v22} = a39);
    const v41 = yield v21;
    await a38;
    const v43 = [v23,v41,964329252n,v5];
    const v44 = [Float64Array,"o",v22];
    [v44,v25,v44,v44,v11];
    [0n,678,3,v41];
    [v14,Float64Array,a38,"localeCompare",v43];
    [C29];
    new Float32Array(678);
    new Uint8Array(248);
    new Uint32Array("fdNr");
    yield "fdNr";
    return a38;
}
f37(58270, v24, v36);
