const v0 = [];
const v1 = [-4294967296,45526];
const v2 = [-128,-1290706317,52589,10,-18733,-1073741824,-6098];
const v3 = [9007199254740992,16];
const v7 = [-9007199254740992,8,7,536870887,-55884];
const v8 = [-11];
const v9 = [130870775,6080,-128];
function f10(a11, a12, a13, a14) {
    const o18 = {
        "f": a11,
        "h": v9,
        get d() {
            a13 >>> v3;
            const o17 = {
                __proto__: v0,
                65536: v1,
            };
            o17.__proto__ = v2;
            return v8;
        },
        "g": -1299217099,
        "c": v7,
        ...v9,
        "a": a12,
    };
    return o18;
}
f10(v0, v0, -1299217099, 9);
f10(v3, f10(v7, v1, 9, 9), 536870888, 536870888);
[10n,-651476.4753644922,-481375.0738250828,2147483647n,63214n];
const v32 = [10n];
const v33 = [672.8710232218446,v32,63214n,-481375.0738250828,-481375.0738250828];
function f34() {
    return v32;
}
class C35 {
    constructor(a37, a38) {
        super.h = a37;
        const v40 = Reflect.iterator;
        const o61 = {
            [v40]() {
                let v42 = 10;
                const o56 = {
                    next() {
                        try {
                        const t0 = 2147483647n;
                        t0(-651476.4753644922, ...v42, 10n, this);
                        } catch (e) {}
                        f34();
                        function f46(a47) {
                            return v42;
                        }
                        class C48 extends f46 {
                            d;
                        }
                        v42--;
                        const v54 = v42 == -1717882815;
                        const o55 = {
                            "done": v54,
                            "value": v42,
                        };
                        return o55;
                    },
                };
                this.__proto__ = o56;
                try { new a37(this, Reflect, o56, o56, Reflect); } catch (e) {}
                return o56;
            },
        };
    }
}
new C35(v32, 672.8710232218446);
const v63 = new C35(v32, -651476.4753644922);
let v64 = new C35(v33, f34);
new Float32Array(3701, v33, Float32Array);
const v70 = new Int32Array();
const v71 = new Int32Array();
function F72(a74) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v71;
    this.e = v70;
}
new Int32Array(6);
new Float32Array(0);
[-Infinity];
const o82 = {
    set e(a81) {
    },
};
with (v63) {
    typeof f === "bigint";
}
for (let v87 = 0; v87 < 5; v87++) {
    o82.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v91 = [13,9,-12559,-8,-4294967297];
[65535,-1139741600];
new Uint8Array(v91);
let v95 = -41990;
const v96 = v64++;
let v97 = --v95;
function f98() {
    return f98;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v97 * (v96 << v95);
