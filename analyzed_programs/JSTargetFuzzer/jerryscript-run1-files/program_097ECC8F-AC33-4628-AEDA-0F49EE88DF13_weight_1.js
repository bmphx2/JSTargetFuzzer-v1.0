-Infinity;
new Int32Array(1);
new Uint8ClampedArray(1024);
new Uint16Array(1609);
new Int8Array(168);
new Int32Array(9);
new Int16Array(5);
new Set();
new WeakMap();
new WeakSet();
const v47 = new URIError(512);
const v50 = new Int16Array(447);
new Uint32Array(2438);
function f57() {
}
v47.description <<= 2006448021;
const v65 = [2.220446049250313e-16,-1.7976931348623157e+308,-3.4833326659982795,-1000000000000.0,0.0,2.220446049250313e-16,-4.191721618721404,1.4104645033139904e+308];
("1073741824").match(v65.copyWithin(v65, v65));
new Uint8Array(0);
const v75 = [60501,11167,2147483649,-1549101807,1073741824,-1795015778,8150,0,-88583214];
let v76 = RegExp.bind("2147483647", v75);
const v78 = new BigUint64Array(127);
function f79(a80, a81) {
    const o82 = {
        "growable": a80,
        ...v76,
        "e": a81,
    };
    return o82;
}
f79(v76, 26357);
f79(v75, 26357);
f79(v65, 5);
v76 /= v78;
new WeakSet();
new BigInt64Array(2, -14, v50, BigInt64Array, v76);
function F97(a99, a100, a101) {
    if (!new.target) { throw 'must be called with new'; }
}
const v102 = new F97();
with (v102) {
    let v103 = 536870887 in v102;
    v103 >>>= v103;
}
([5]).reverse();
