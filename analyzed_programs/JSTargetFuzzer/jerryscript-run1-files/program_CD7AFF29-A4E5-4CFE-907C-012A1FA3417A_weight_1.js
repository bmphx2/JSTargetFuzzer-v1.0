new SyntaxError(SyntaxError);
const v7 = [2n,-6610n,-1290666896n];
const v9 = new BigInt64Array();
switch (v7) {
    case v9:
        break;
}
("-21827")["charCodeAt"]();
const v16 = new Float64Array(4032);
v16.lastIndexOf(1603874941);
function f23(a24, a25, a26) {
    const o36 = {
        n(a28, a29, a30, a31) {
            a28[a26];
            [257,9223372036854775807,-268435456,10,2,-9223372036854775807,-4294967297,1073741825,1073741825,1000];
            [609089358,-1,-2147483648,10,1073741823,4294967296,65535,2480,65537,-128];
            [1024,65535,1024,16,4,49676,-2147483648,-5,57864,-10355];
            return a25;
        },
        "a": 6,
        ...a24,
        ..."description",
        "g": "__proto__",
        "e": "__proto__",
        [a24]: a25,
    };
    return a25;
}
const v37 = f23("__proto__", 6, -27883);
function F39(a41, a42, a43) {
    if (!new.target) { throw 'must be called with new'; }
}
new F39(v37);
try { ("b").codePointAt(-63267); } catch (e) {}
const v49 = !536870912;
Math.sin(-63267);
const v51 = -565289.7889622233 * v49;
~(~"__proto__");
536870912 || v51;
const v58 = new Uint32Array(3164);
function F59(a61, a62, a63, a64) {
    if (!new.target) { throw 'must be called with new'; }
    this.g = v58;
    this.e = v7;
}
new BigUint64Array(4032);
function f67() {
    let {...v69} = arguments;
    return 12;
}
new Float64Array(12);
new BigInt64Array(6);
let v76 = 10;
for ((() => {
        class C77 {
        }
    })();
    v76--;
    ) {
}
const v79 = new Uint8ClampedArray();
function f80(a81, a82, a83) {
    return a82;
}
for (const v88 in +(4 / Int8Array)) {
}
try { v79["set"](4096, 23, ...v79, ...23); } catch (e) {}
for (const v93 in Float64Array) {
}
