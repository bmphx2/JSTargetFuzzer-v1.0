function f3(a4) {
    const o19 = {
        o(a6) {
            try {
                const v7 = super.valueOf();
                try { v7(1261525374, v7, this, 1261525374, v7); } catch (e) {}
                const v12 = a6 ^ -9223372036854775808;
                Math.log1p(v12);
                Math.pow(a6, a4);
                Math.asin(v12);
                Math.sign(268435439);
                a6 % v12;
            } catch(e18) {
            }
            return this;
        },
        "h": 65536,
        "e": 65536,
        "a": 1261525374,
    };
    return o19;
}
f3(1261525374);
f3(257);
const v22 = f3(1261525374);
function F23(a25, a26) {
    if (!new.target) { throw 'must be called with new'; }
    v22.c = a25;
}
const v27 = new F23(F23, 65536);
const v28 = new F23(f3, v27);
new F23(f3, v28);
new Float32Array(3701);
const v35 = new Int32Array();
const v36 = new Int32Array();
function F37(a39) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v36;
    this.e = v35;
}
new Int32Array(6);
new Float32Array(0);
const v44 = [-Infinity];
const o47 = {
    set a(a46) {
    },
};
for (let v48 = 0; v48 < 5; v48++) {
    o47.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v52 = [13,9,-12559,-8,-4294967297];
function f56() {
    return 52639;
}
let v57 = [65535,-1139741600];
new Uint8Array(v52);
let v60 = -41990;
const v61 = v57++;
let v62 = --v60;
function f63() {
    return Float32Array;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v62;
v44 * (v61 << 1261525374);
