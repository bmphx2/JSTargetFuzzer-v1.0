function f0() {
}
const v1 = [-11,-65535,-269244937,-224158492,1625985610,268435456,2147483648,40674,-853865423];
const v2 = [-12,-128,1,1073741825,-4];
const v3 = [-1418664377,5,2147483648];
function F4(a6, a7) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v1;
}
const v8 = new F4(f0, v3);
const v9 = new F4(v8, v3);
new F4(v9, v3);
new Int8Array(1);
new Uint16Array(1);
new Uint8ClampedArray(1000);
new Int8Array(1000);
new Int32Array(9);
new Int16Array(1000);
new Set();
new WeakMap();
new WeakSet();
const v55 = new URIError(268435440);
const v58 = new Int16Array(447);
new Uint32Array(2438);
function f65() {
}
v55.description <<= 129;
const v73 = [2.220446049250313e-16,-1.7976931348623157e+308,-3.4833326659982795,-1000000000000.0,0.0,2.220446049250313e-16,-4.191721618721404,1.4104645033139904e+308];
v73.copyWithin(v73, v2);
("1073741824").match(v2);
new Uint8Array(1000);
[-8,4294967295,148900462];
let v84 = RegExp.bind("2147483647", v3);
new BigUint64Array(127);
function f87(a88, a89) {
    const o90 = {
        "f": F4,
        ...a88,
        "e": a89,
    };
    return o90;
}
f87(63703n, 2);
f87(v3, 26357);
f87(v73, 2);
v84 /= v84;
new WeakSet();
new BigInt64Array(2, 1, v58);
([5]).reverse();
