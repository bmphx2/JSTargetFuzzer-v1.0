const v1 = new Map();
const v2 = [v1];
const v3 = [v1];
const v4 = [v2,v3,v3,v3];
v2 != v2;
let v11 = v2 ^ "__proto__";
function f13() {
    v11 = 2;
    for (let v14 = 0; v14 < 32; v14++) {
        v3["p" + v14] = v14;
    }
    return v4;
}
[3.0,-6.142582309748291e+306,1000.0];
[2.220446049250313e-16];
[-625773.6824483686,2.0,Infinity];
new Date();
function F37() {
    if (!new.target) { throw 'must be called with new'; }
    F37.prototype = 1433387171;
}
const v40 = new F37();
const v41 = new F37();
const v42 = new F37();
function f44() {
    const v47 = new Uint32Array(858);
    return v47;
}
function f50() {
    return v40;
}
const v53 = new BigUint64Array(BigUint64Array, BigUint64Array);
v53.copyWithin();
const v57 = RegExp.bind(2);
v41[256] = v4;
Date(4294967297);
const t35 = "__proto__";
t35.length ^= 268435441;
function f60() {
    return v40;
}
Math.fround();
const v67 = new BigUint64Array();
v67.fill(BigInt64Array < 11);
Math.min(NaN);
let v71 = 1073741824n;
v71 &= v71;
try { Float32Array(); } catch (e) {}
const o76 = {
    "e": "__proto__",
    148482391: v42,
};
(-12n).toString();
([2036046200,-9223372036854775807,-61646])["map"](Array);
new Float64Array(16);
class C89 {
}
const v91 = new v57(1);
const v94 = new Int8Array(107);
for (let i97 = 0, i98 = 10; i97 < i98;) {
    v94[v91] = i97;
    ++i97;
    Math.max(7);
}
new Int16Array(26);
4 == [4,7,4294967297,-1,1000];
const v114 = new Int8Array(Int8Array, Int8Array, Int8Array);
v114.includes(v114, v114);
new Uint8ClampedArray(6);
Math.pow(-343821812, -343821812);
Math.random();
Math.sign(-1.3649564016016038e+308);
Math.min(-1.3649564016016038e+308);
try {
    new WeakSet(-13n);
} catch(e129) {
} finally {
}
