-Infinity;
new Int32Array(1);
new Uint8ClampedArray(1024);
new Uint16Array(1609);
new Int8Array(168);
new Int32Array(9);
new Int16Array(5);
new Set();
new WeakMap();
1n == 886.0009926550579;
Symbol.species.toString();
new WeakSet();
async function f43(a44, a45) {
    class C46 extends a45 {
    }
    return C46;
}
const v57 = new URIError(512);
const v60 = new Int16Array(447);
new Uint32Array(2438);
function f67() {
}
v57.description <<= 2006448021;
const v75 = [2.220446049250313e-16,-1.7976931348623157e+308,-3.4833326659982795,-1000000000000.0,0.0,2.220446049250313e-16,-4.191721618721404,1.4104645033139904e+308];
const v76 = v75.copyWithin(v75, v75);
function f78(a79, a80) {
    return Uint8ClampedArray;
}
for (let v81 = 0; v81 < 32; v81++) {
}
("1073741824").match(v76);
new Uint8Array(0);
const v90 = [60501,11167,2147483649,-1549101807,1073741824,-1795015778,8150,0,-88583214];
let v91 = RegExp.bind("2147483647", v90);
const v93 = new BigUint64Array(127);
function f94(a95, a96) {
    const o97 = {
        "growable": a95,
        ...v91,
        "e": a96,
    };
    2.220446049250313e-16 - 11;
    return o97;
}
f94(v91, 26357);
f94(v90, 26357);
f94(v75, 5);
v91 /= v93;
new WeakSet();
new BigInt64Array(2, -14, v60, BigInt64Array, v91);
function F115(a117, a118, a119) {
    if (!new.target) { throw 'must be called with new'; }
}
const v120 = new F115();
with (v120) {
    let v121 = 536870887 in v120;
    v121 >>>= v121;
}
([5]).reverse();
