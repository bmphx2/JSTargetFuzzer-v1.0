const v6 = [2];
const v7 = [9,7,4,10000,-466611057,83963959,-2,7];
const v8 = [255,9,536870888,8,-55066];
const v12 = v6["g"];
function F13(a15, a16, a17, a18) {
    if (!new.target) { throw 'must be called with new'; }
    this.d = a15;
}
new F13(v7, "unscopables", "register", v12, F13, v7);
new F13(v12, "unscopables", "valueOf", 1024n);
new F13(v8, "flatMap", v12, -1542972315n);
const v26 = -Infinity;
[10n,-651476.4753644922,-481375.0738250828,2147483647n,63214n];
const v29 = [10n];
const v30 = [v26,v29,63214n,-481375.0738250828,-481375.0738250828];
function f31() {
    return v29;
}
class C32 {
    constructor(a34, a35) {
        super.h = a34;
        const v37 = Symbol.iterator;
        const o60 = {
            [v37]() {
                let v39 = -56772;
                const o55 = {
                    set d(a41) {
                        a35[9] = this;
                    },
                    "g": v39,
                    "d": v39,
                    next() {
                        try {
                        const t0 = 2147483647n;
                        t0(-651476.4753644922, ...v39, 10n, this);
                        } catch (e) {}
                        f31();
                        function f45(a46) {
                            return v39;
                        }
                        class C47 extends f45 {
                            d;
                        }
                        v39--;
                        const v53 = v39 == 0;
                        const o54 = {
                            "done": v53,
                            "value": v39,
                        };
                        return o54;
                    },
                };
                this.__proto__ = o55;
                try { new a34(this, Symbol, o55, o55, Symbol); } catch (e) {}
                return o55;
            },
        };
    }
}
new C32(v29, v26);
const v62 = new C32(v29, -651476.4753644922);
let v63 = new C32(v30, f31);
new Float32Array(3701);
const v69 = new Int32Array();
const v70 = new Int32Array();
function F71(a73) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v70;
    this.e = v69;
}
new Int32Array(6);
new Float32Array(0);
[-Infinity];
const o81 = {
    set a(a80) {
    },
};
with (v62) {
    typeof f === "bigint";
}
for (let v86 = 0; v86 < 5; v86++) {
    o81.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v90 = [13,9,-12559,-8,-4294967297];
[65535,-1139741600];
new Uint8Array(v90);
let v94 = -41990;
const v95 = v63++;
let v96 = --v94;
function f97() {
    return f97;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v96 * (v95 << v94);
