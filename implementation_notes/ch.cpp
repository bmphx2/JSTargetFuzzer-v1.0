...
//extern "C" void __sanitizer_cov_trace_pc_guard_bkp(uint32_t *guard) {
...
extern "C" void __sanitizer_cov_trace_pc_guard(uint32_t *guard) {
    ...
    //void* targetMemory = (void*) 0x55555575d3cf; //0x555555749254; //GlobalObject.cpp 0x5555565cfffd 0x555555d65c72 DoublyLinkedListElement.inl
    //void *targetMemoryList[] = {(void*)0x5555559c7580, (void*)0x55555783bd0, (void*)0x55555575d3cf, (void*)0x555556ce0e66};
    void *targetMemoryList[] = {(void*)0x555556ce0e66}; // 0x555556ce0e66 0x555556cd8790 x555556ce0e66
    //Lower.cpp, JavaScriptArray.cpp, JavaScriptOperators.cpp, Parse.cpp

    size_t listSize = sizeof(targetMemoryList) / sizeof(targetMemoryList[0]);


    //char PcDescr[1024];
    //0xff

    //considering the first four-bytes

    uintptr_t mask = 0xFFFFFFFFFFFF0000;
    uintptr_t maskedPC = (uintptr_t)PC & mask;
    //__shmem->maskedPC = (uintptr_t)PC & mask;
    //__shmem->maskedTarget = (uintptr_t)targetMemory & mask;

    //__sanitizer_symbolize_pc(PC, "%p %F %L", PcDescr, sizeof(PcDescr));
    //printf("guard: %p %x PC %s\n", guard, *guard, PcDescr);
 
    //uint32_t PC_int = (uint32_t)(((uintptr_t)PC) >> 16);
    //uint32_t targetMemory_int =(uint32_t)(((uintptr_t)targetMemory) >> 16);

    //uint32_t PC_int = (uint32_t)((uintptr_t)PC);
    //uint32_t targetMemory_int = (uint32_t)((uintptr_t)targetMemory);

    //printf("%x\n", PC_int);
    //printf("%x\n", targetMemory_int);
    //__shmem->found_sec_cov = false;

    // qual valor do edge
    //__curedges->current_edges[index / 8] =  __shmem->edges[index / 8];
    //if ((__shmem->edges[index / 8] & (1 << (index % 8))) == 0) {
    //if ((__shmem->found_edges < ))
    //    cur_edge = true;
    //}
    
  
    __shmem->edges[index / 8] |= 1 << (index % 8);
    //__shmem->found_seccov = 0;
    //__shmem->seccov[index / 8] |= 1 << (index % 8);
    
    //printf("PC: %p\n", targetMemory);
    //if (PC_int == targetMemory_int) {
    //if (PC == targetMemory) {
    //    __shmem->edges[index / 8] |= 1 << ((index + 1) % 8);
    //    __shmem->found_sec_cov = true;
    //    __shmem->num_sec_cov = __shmem->num_sec_cov + 1;
        //printf("found one\n");
        //printf("number of found cov: %i\n", __shmem->num_sec_cov);
        //address_count++;
    //}

    //for (size_t i = 0; i < listSize; i++) {
        //uint32_t targetAddressUpperBits = (uint32_t)(((uintptr_t)PC) >> 48);//16);
        //uint32_t listAddressUpperBits = (uint32_t)(((uintptr_t)targetMemoryList[i]) >> 48); //16);
    //    uintptr_t maskedTargetMemory = (uintptr_t)targetMemoryList[i] & mask;
    //    __shmem->maskedTarget = (uintptr_t)targetMemoryList[i] & mask;

        // if valor_ant = 0 new edge
    //    if ((cur_edge == true)&&(maskedPC == maskedTargetMemory)) {
        //if (targetAddressUpperBits == listAddressUpperBits) {
    //        __shmem->num_sec_cov = __shmem->num_sec_cov + 1;
    //        __shmem->found_sec_cov = true;
            //printf("Return address: %p & Target memory: %p\n" , maskedPC, maskedTargetMemory);
            //printf("Found one\n");
    //    }
    //}
    ...
    for (size_t i = 0; i < listSize; i++) {
        ...
        if (maskedPC == maskedTargetMemory) {
    
            //__shmem->found_seccov = false;
            //__shmem->seccov[index / 8] |= 1 << (index % 8);
            ...
            if (isNewAddress) {
                ...
                printf("New unique address stored: %p\n", (uintptr_t)targetMemoryList[i]);
                printf("Total unique addresses: %i\n", __shmem->usedCount);
            }
