new BigUint64Array(7);
new BigInt64Array(29);
new Uint16Array(255);
new Float32Array(3701);
const v17 = new Int32Array();
const v18 = new Int32Array();
function F19(a21) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v18;
    this.e = v17;
    let v23 = new Int16Array(Int16Array);
    ({"byteLength":v23,...v23} = v23);
    const v25 = new Uint16Array(Uint16Array, Uint16Array, Uint16Array);
    const v27 = new Float32Array(v25, v25, v25);
    v25.copyWithin(197, v27.length);
}
new Int32Array(6);
function F33() {
    if (!new.target) { throw 'must be called with new'; }
}
const v35 = new F33();
new F33(F33, v35, ...[12,1237513625,666805480,28845,39666,-2]);
new Float32Array(0);
function F40() {
    if (!new.target) { throw 'must be called with new'; }
}
const v42 = [1.7976931348623157e+308];
function* f43(a44, a45) {
    yield* v42;
    return WeakSet();
}
[-Infinity];
let v49 = 1n;
v49 &= 4n;
const v52 = new BigInt64Array();
v52.keys();
const o62 = {
    set a(a55) {
        for (let v57 = 0; v57 < 5; v57++) {
            try { Reflect.ownKeys(v57); } catch (e) {}
        }
        ("m")["match"]();
    },
};
for (let v63 = 0; v63 < 5; v63++) {
    o62.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v67 = [13,9,-12559,-8,-4294967297];
let v68 = [65535,-1139741600];
new Uint8Array(v67);
let v71 = -41990;
const v72 = v68++;
let v73 = --v71;
function f74() {
    new Uint32Array(2334);
    const t57 = [1679989717,-4294967297,-10,-3691,12801];
    t57.length = 2;
    return f74;
}
new BigInt64Array();
Symbol.iterator;
const v84 = [Infinity,7.771821347908961e+307,0.47279638134182167,1000000.0,-1000.0,-924781.997015073,6.566625761778173,1000000000000.0,1000000000.0];
const v85 = [-361040.7831500884,-1.7976931348623157e+308,-8.997086225431573];
const v88 = [v85,[13,v84]];
function f89() {
    return v88;
}
class C90 extends f89 {
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v73 * (v72 << v71);
