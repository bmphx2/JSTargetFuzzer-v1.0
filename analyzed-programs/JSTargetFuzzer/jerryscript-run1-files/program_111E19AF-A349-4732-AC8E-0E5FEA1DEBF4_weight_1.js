function F0() {
    if (!new.target) { throw 'must be called with new'; }
    this.b = 268435439;
}
new F0();
new F0();
const v5 = new F0();
function f6() {
    return F0;
}
[0.0,-650.1064846294582,-5.0,-586.9074623014933,2.220446049250313e-16];
const v11 = [6.875884676316055,-1000000.0,2.2250738585072014e-308,-0.0];
[2.0,2.3599613984444208,1.0,-505839.2651053678,6.959673568545938];
new Float32Array(3701);
const v18 = new Int32Array();
const v19 = new Int32Array();
function F20(a22) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v19;
    Object.defineProperty(v5, 2147483649, { configurable: true, enumerable: true, value: Float32Array });
    this[257];
    this.e = v18;
}
new Int32Array(6);
new Float32Array(255);
[-Infinity];
const o34 = {
    set a(a33) {
    },
};
for (let i36 = 0; i36 < 5; i36++) {
    v11[v5.__proto__];
    try { o34.getSeconds(F20); } catch (e) {}
}
for (let v45 = 0; v45 < 5; v45++) {
    o34.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v49 = [13,9,-12559,-8,-4294967297];
[65535,-1139741600];
let v52 = new Uint8Array(v49);
let v53 = -41990;
const v54 = v52++;
let v55 = --v53;
function f56() {
    return f56;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
Boolean([1e-15,9.323130580524804,-2.0,-1.0]);
--v55 * (v54 << v53);
