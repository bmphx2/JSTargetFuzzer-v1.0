function F3(a5) {
    if (!new.target) { throw 'must be called with new'; }
    this.b = 1460398922n;
}
const v6 = new F3(1460398922n);
new F3(129n);
const v8 = new F3(34555n);
const v14 = new Uint8ClampedArray(54);
new Float64Array(3);
new Int8Array(7);
const v25 = new Date();
v14[v8] = -65535n;
Object.defineProperty(v8, "a", { get: Date, set: Date });
function F38() {
    if (!new.target) { throw 'must be called with new'; }
    F38.prototype = 1433387171;
}
const v41 = new F38();
new F38();
new F38();
function f45() {
    const v48 = new Uint32Array(858);
    return v48;
}
function f50(a51, a52, a53) {
    f = a51;
    return a51;
}
f50(v25, "a", v6);
function f57() {
    return v41;
}
for (let v59 = 0; v59 < 62; v59++) {
    `object${v59}object`;
    break;
}
const v65 = new BigUint64Array(BigUint64Array, BigUint64Array);
v65.copyWithin();
const v69 = RegExp.bind(2);
Math.fround();
Math.min(NaN);
let v75 = 1073741824n;
v75 &= v75;
try { Float32Array(); } catch (e) {}
(-12n).toString();
([2036046200,-9223372036854775807,-61646])["map"](Array);
new Float64Array(16);
class C89 {
}
const v91 = new v69(1);
const v94 = new Int8Array(107);
for (let i97 = 0, i98 = 10; i97 < i98;) {
    v94[v91] = i97;
    ++i97;
    Math.max(7);
}
new Int16Array(26);
4 == [4,7,4294967297,-1,1000];
const v114 = new Int8Array(Int8Array, Int8Array, Int8Array);
v114.includes(v114, v114);
new Uint8ClampedArray(6);
try {
    new WeakSet(-13n);
} catch(e121) {
} finally {
}
