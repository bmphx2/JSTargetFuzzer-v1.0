const v0 = [];
function f4() {
    return "rMf";
}
new Int32Array();
const v7 = [461,-33280,65537];
const v8 = [4,9007199254740991,-1183379087,536870887,-5,-13,16,-1,65537,129];
v8.length = 256;
v8.unshift(v7);
const v11 = ["catch","catch",f4,"catch","catch"];
const v12 = [f4,"rMf",v11,v0,v11];
const v13 = [v12,"catch","catch",v12,v12];
function F14(a16, a17) {
    if (!new.target) { throw 'must be called with new'; }
    this.c = a17;
    this.e = a17;
}
const v18 = new F14(v0, "rMf");
const v19 = new F14(v13, v18);
for (; !256;) {
}
new F14(v0, v19);
const v24 = new Float32Array(3701);
const v27 = new Int32Array();
const v28 = [-159.91632461601353,1.2939760484235483e+308,-234.44839619711706,4.0,1e-15,5.0,-5.0,448.93894136360746,-629237.2752174904,-1.5485730665822505e+308];
for (let v29 = 0; v29 < 5; v29++) {
    v28["p" + v29] = v29;
}
const v32 = new Int32Array();
function F33(a35) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v32;
    this.e = v27;
}
new Int32Array(6);
let v38 = [65535,-1139741600];
v38++ << -41990;
new Float64Array(3658);
new Float32Array(3701);
[-Infinity];
const o50 = {
    set a(a49) {
    },
};
for (let v51 = 0; v51 < 5; v51++) {
    o50.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
let v55 = -8n;
v55 &= 536870888n;
class C58 extends Date {
    constructor(a60, a61) {
        super(a61);
    }
}
const v62 = [13,9,-12559,-8,-4294967297];
let v63 = [65535,-1139741600];
new Uint8Array(v62);
let v66 = -41990;
v63++;
let v68 = --v66;
function f69() {
    return f69;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v68 * (v24 << v66);
