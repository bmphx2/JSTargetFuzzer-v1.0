const v6 = [10000,-22543,8];
const v7 = [-910348374,-9223372036854775808,-8,-65537,38760];
const v8 = [0,-9007199254740991,643680889,-12,129,1923840396,5,45520,-2147483647];
function F9(a11, a12, a13) {
    if (!new.target) { throw 'must be called with new'; }
    this.e = 10n;
}
new F9(-28371n, 10n, v7);
const v15 = new F9(8n, -28371n, 6427);
const v16 = new F9(8n, 10n, v6);
function f17(a18, a19, a20, a21) {
    const o34 = {
        __proto__: v8,
        set d(a23) {
            delete this[1000];
            new WeakSet();
        },
        [v6](a28, a29, a30) {
            a30.toString = a19;
            return 14;
        },
        ...v15,
    };
    return o34;
}
f17(v16, v8, v6, v7);
f17(f17(10n, f17, v8, v8), 8n, v8, v7);
new Float32Array(3701);
const v43 = new Int32Array();
const v44 = new Int32Array();
function F45(a47) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v44;
    this.e = v43;
}
new Int32Array(6);
const v51 = [1000000.0];
for (let v52 = 0; v52 < 32; v52++) {
    v51["p" + v52] = v52;
}
Object.freeze();
new Float32Array(0);
[-Infinity];
const o61 = {
    set a(a60) {
    },
};
for (let v62 = 0; v62 < 5; v62++) {
    o61.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v66 = [13,9,-12559,-8,-4294967297];
let v67 = [65535,-1139741600];
new Uint8Array(v66);
let v70 = -41990;
const v71 = v67++;
let v72 = --v70;
function f73() {
    return f73;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v72 * (v71 << v70);
