new Int16Array(7);
new BigUint64Array(3781);
for (let i11 = 0; i11 < 9; i11++) {
    new BigUint64Array(i11, 9007199254740991, i11);
}
new Int32Array(1024);
[-128,2147483649,10,10,128,-9603,-2,268435441];
[-1441011915,-1477473335,255];
[8,-11,64,1000,-5007,1714672298];
const v25 = new Float32Array(3701);
for (let v28 = 0; v28 < 32; v28++) {
    v25["p" + v28] = v28;
}
const v31 = new Int32Array();
const v32 = new Int32Array();
function F33(a35) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v32;
    try { this["parse"]("parse", this, v31); } catch (e) {}
    const o38 = {
    };
    new Proxy("parse", o38);
    this.e = v31;
}
new Int32Array(6);
new Float32Array(0);
[-Infinity];
const o48 = {
    set a(a47) {
    },
};
for (let v49 = 0; v49 < 5; v49++) {
    o48.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v53 = [13,9,-12559,-8,-4294967297];
let v54 = [65535,-1139741600];
new Uint8Array(v53);
let v57 = -41990;
const v58 = v54++;
let v59 = --v57;
function f60() {
    return f60;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v59 * (v58 << v57);
