const v0 = [4.2164803288028424e+307,3.0];
const v1 = [0.5982808497639508];
const v2 = [-1.7390674947627218,3.0,741496.051964398,-100.91723897722636,0.0,-338411.7952810939,9.766644180954788,1.7431794465812224e+308,4.975894344760492,1.0];
let v6;
try {
const t0 = 6681;
v6 = new t0();
} catch (e) {}
let {"a":v7,"b":v8,"length":v9,} = v1;
new BigUint64Array(208);
new Uint8Array(512);
new Int8Array(565);
function f19(a20, a21) {
    const o28 = {
        "d": v0,
        230: 6681,
        ...a21,
        n(a23, a24) {
            a24[a24 !== a21 ? a24 : a21];
            return this;
        },
        "a": -1591506811,
    };
    return o28;
}
const v29 = f19(v1, v2);
const v30 = f19(v1, v2);
f19(v2, v2);
const v34 = new Float32Array(3701);
const v37 = new Int32Array();
const v38 = new Int32Array();
function F39(a41) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v38;
    this.e = v37;
}
let v42 = new Int32Array(6);
try { v42.forEach(f19, v34); } catch (e) {}
({"d":v42,} = v30);
const v44 = new Float32Array(v37, 6681, 6);
new Float32Array(v30, 6681);
new Float32Array(0);
const v52 = [-Infinity];
const o59 = {
    set a(a54) {
        class C55 {
            [41126] = a54;
        }
        new C55();
        new C55();
        new C55();
    },
};
for (let v60 = 0; v60 < 5; v60++) {
    new f19(v29, v60);
    new Uint8ClampedArray(7);
    new Int8Array(29);
    new Uint8Array(16);
    o59.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v74 = [13,9,-12559,-8,-4294967297];
let v75 = [65535,-1139741600];
new Uint8Array(v74);
let v78 = -41990;
const v79 = v75++;
let v80 = --v78;
if (v52 > v0) {
    new Uint8ClampedArray(74);
    new Uint32Array(102);
    new Uint8ClampedArray(128);
} else {
    try { v8(v44, v6, 208, 0, 41126); } catch (e) {}
}
function f92() {
    return f92;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v80 * (v79 << v78);
