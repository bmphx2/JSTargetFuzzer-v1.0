const v2 = new BigInt64Array(6);
new Uint8ClampedArray(9);
const v8 = new Int16Array(87);
function F9(a11, a12, a13) {
    if (!new.target) { throw 'must be called with new'; }
    v8.c = a12;
}
new F9(6, 87, F9);
new F9(87, 6, Uint8ClampedArray);
const v16 = new F9(9, 9, 9);
new Float32Array(58410);
v16[1444551605] = v16;
new Int16Array(128, BigInt64Array, 6, 127, 3701);
new BigInt64Array(9, 3701);
function f26() {
    return v2;
}
const v28 = new Int32Array();
const v29 = new Int32Array(v28);
function F30(a32) {
    if (!new.target) { throw 'must be called with new'; }
    for (let i35 = 0, i36 = 10; i35 < i36; i35++, i36--) {
        [] = v2;
        try { this.p(Int16Array, this); } catch (e) {}
    }
    this.f = v29;
    this.e = v28;
}
new Int32Array(6);
new Float32Array(0);
[-Infinity];
const o52 = {
    set a(a51) {
    },
};
for (let v53 = 0; v53 < 5; v53++) {
    o52.a;
}
let v55 = [-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v57 = [13,9,-12559,-8,-4294967297];
[65535,-1139741600];
new Uint8Array(v57);
let v61 = -41990;
const v62 = v55++;
let v63 = --v61;
function f64() {
    return Int32Array;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
F30 * (v62 << --v63);
