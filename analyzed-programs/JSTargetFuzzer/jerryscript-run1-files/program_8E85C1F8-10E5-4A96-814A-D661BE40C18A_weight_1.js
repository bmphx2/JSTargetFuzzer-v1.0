let v1 = 2.220446049250313e-16;
v1 = -1000000000000.0;
const v3 = 3.0 & -1000000000000.0;
const v4 = [v3,v3,v3,v3];
[3.0,3.0];
[-1000000000000.0,v4,v4,v1,v4];
function F7(a9, a10) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = a9;
    this.h = v1;
}
const v11 = new F7(3.0, v1);
const v12 = new F7(-1000000000000.0, v1);
const v13 = new F7(v1, 3.0);
function F14(a16, a17, a18, a19) {
    if (!new.target) { throw 'must be called with new'; }
    this.a = a19;
}
const v20 = new F14(-1000000000000.0, v1, v1, v11);
new F14(-1000000000000.0, 3.0, v20, v13);
new F14(-1000000000000.0, 3.0, v13, v12);
new Float32Array(3701);
new Int8Array(16);
new Float64Array(11);
new Uint32Array(16);
const v40 = new Int32Array();
const v41 = new Int32Array();
function F42(a44) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v41;
    this.e = v40;
}
new Int32Array(6);
new Float32Array(0);
[-Infinity];
const o52 = {
    set a(a51) {
    },
};
for (let v53 = 0; v53 < 5; v53++) {
    o52.a;
}
let v55 = [-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v57 = [13,9,-12559,-8,-4294967297];
[65535,-1139741600];
new Uint8Array(v57);
let v61 = -41990;
v55++;
let v63 = --v61;
function f64() {
    return f64;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v63 * (11 << v61);
