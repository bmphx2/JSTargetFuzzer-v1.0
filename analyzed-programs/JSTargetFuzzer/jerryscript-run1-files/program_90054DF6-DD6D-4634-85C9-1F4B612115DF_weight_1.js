const v0 = [15,8,1024,0,-4096];
const v1 = [0,16,946856160,-1];
const v2 = [-10,12];
function f3() {
    return v2;
}
!f3;
new Array(19);
class C11 {
    e = 0.3875391464451593;
    333063374 = 0.0;
    static o(a13, a14, a15) {
        a14.length = v0;
        v1[0.6464240545529928] = 0.3875391464451593;
        return v1;
    }
    p(a17, a18) {
        function f19() {
            return f19;
        }
        new Float64Array(3538);
        const v23 = a17 && a17;
        let v24;
        try { v24 = a17(f3, v23); } catch (e) {}
        v24.c = 3538;
        super.a = 0.6464240545529928;
        const o25 = {
            "apply": Array,
            "construct": f3,
            "deleteProperty": f3,
            "getOwnPropertyDescriptor": Array,
            "isExtensible": f19,
            "ownKeys": f19,
            "set": Array,
        };
        new Proxy(v2, o25);
        const v28 = typeof v0;
        v28 === "string";
        return v28;
    }
}
new C11();
new C11();
new C11();
new Float32Array(3701);
const v42 = new Int32Array();
const v43 = new Int32Array();
function F44(a46) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v43;
    this.e = v42;
}
new Int32Array(6);
new Float32Array(0);
[-Infinity];
const o54 = {
    set a(a53) {
    },
};
for (let v55 = 0; v55 < 5; v55++) {
    o54.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v59 = [13,9,-12559,-8,-4294967297];
let v60 = [65535,-1139741600];
new Uint8Array(v59);
let v63 = -41990;
const v64 = v60++;
let v65 = --v63;
function f66() {
    return f66;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v65 * (v64 << v63);
