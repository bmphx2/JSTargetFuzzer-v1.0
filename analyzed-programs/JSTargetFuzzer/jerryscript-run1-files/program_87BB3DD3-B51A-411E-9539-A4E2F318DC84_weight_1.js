const v6 = [-8.879395471140676];
const v7 = [-1.9525685937744353,1e-15,-3.8033568064167973,694.4666557256508,4.0904136904884165,-4.0];
const v8 = [1.4427872472051034e+308,-1000.0,1.492965020090697,-8.273280514521046,1e-15,882688.6065192174,1000000000.0];
new WeakSet();
function F14(a16, a17, a18) {
    if (!new.target) { throw 'must be called with new'; }
    this.e = a16;
}
const v19 = new F14(v8, F14, -13n);
new F14(v7, v19, 536870912n);
new F14(v8, v6, -13n);
new Float32Array(3701);
const v27 = new Number();
const v28 = new Number();
function F29(a31) {
    if (!new.target) { throw 'must be called with new'; }
    this.h = v28;
    this.e = v27;
}
new Number(6);
new Float32Array(0);
[-Infinity];
const o39 = {
    set a(a38) {
    },
};
for (let v40 = 0; v40 < 5; v40++) {
    o39.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v44 = [13,9,-12559,-8,-4294967297];
let v45 = [65535,-1139741600];
new Uint8Array(v44);
let v48 = -41990;
const v49 = v45++;
let v50 = --v48;
function f51() {
    return f51;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v50 * (v49 << v48);
