function f6() {
    return -8.075201099382367;
}
function F7(a9, a10, a11) {
    if (!new.target) { throw 'must be called with new'; }
    this.g = -8.075201099382367;
}
new F7(-471935.0889197431, -471935.0889197431, 8);
new F7(-8.075201099382367, -8.075201099382367, -9223372036854775808);
const v15 = new Uint8Array();
const v18 = Math.sign(9);
try { v15.set(v15, v18); } catch (e) {}
new F7(-1.0, -1.0, -1073741824);
class C22 {
}
function F23() {
    if (!new.target) { throw 'must be called with new'; }
}
const v25 = new F23();
with (v25) {
    const v26 = `
        function F27(a29, a30) {
            if (!new.target) { throw 'must be called with new'; }
        }
    `;
    eval(v26);
}
new Float32Array(3701);
const v37 = new Int32Array();
const v38 = new Int32Array();
function F39(a41) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v38;
    this.e = v37;
}
new Int32Array(6);
new Float32Array(0);
[-Infinity];
const o49 = {
    set a(a48) {
    },
};
for (let v50 = 0; v50 < 5; v50++) {
    o49.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v54 = [13,9,-12559,-8,-4294967297];
let v55 = [65535,-1139741600];
new Uint8Array(v54);
let v58 = -41990;
const v59 = v55++;
let v60 = --v58;
function f61() {
    return f61;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v60 * (v59 << v58);
