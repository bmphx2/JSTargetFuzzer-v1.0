class C3 {
    [1433148593];
    p(a5) {
        this[3] = this;
        const v6 = "name" === 2147483647;
        if (v6) {
            [this,1433148593,a5,v6];
            [v6,[this],"name"];
        } else {
            const t9 = "name";
            t9.e = this;
            const o10 = {
            };
            new Proxy("name", o10);
        }
        return this;
    }
}
const o15 = {
    "maxByteLength": 16,
};
const v17 = new ArrayBuffer(0, o15);
new Float32Array(v17);
new C3();
new C3();
new C3();
[1073741824,9223372036854775807,127,-20730,-51101,4];
[268435456,0,10,-256,-9223372036854775807,-9223372036854775808,-22998,1000,256];
[-4,2,-1,11,9007199254740990,1073741824,-275183518];
new Float32Array(3701);
const v37 = new Int32Array();
const v38 = new Int32Array();
for (const v39 of "name") {
}
function F43(a45) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v38;
    this.e = v37;
}
new Int32Array(6);
new Float32Array(0);
[-Infinity];
const o53 = {
    set a(a52) {
    },
};
for (let v54 = 0; v54 < 5; v54++) {
    o53.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v58 = [13,9,-12559,-8,-4294967297];
let v59 = [65535,-1139741600];
new Uint8Array(v58);
let v62 = -41990;
const v63 = v59++;
let v64 = --v62;
function f65() {
    return f65;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v64 * (v63 << v62);
