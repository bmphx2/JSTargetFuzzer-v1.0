const v0 = [-536870912,1,-4096,65536,32949,8];
const v1 = [536870887,146593384,1968398453,-11,6,-9007199254740992,72443636,16];
[536870888,-1665770758,6,6,6,4,-2,2147483649,-1790037133,14881254];
const v6 = [-1757126558,v1,v0];
new Float64Array(255);
new Int16Array(512);
new Float64Array(9);
[[-1757126558,v6,v0,21714]];
new Float32Array(255);
new Uint8ClampedArray(64);
const v28 = new Uint16Array(3638, 3638);
const v31 = new Uint8ClampedArray(16);
const v32 = new Uint8ClampedArray(v31);
v28.slice(3638);
const v34 = v32[10];
try { v34.toString(v34); } catch (e) {}
new Float64Array(6);
function F37() {
    if (!new.target) { throw 'must be called with new'; }
    F37.c = -4294967297;
}
const v40 = new F37();
new F37();
new F37();
function f44() {
    const v47 = new Uint32Array(858);
    return v47;
}
function f50() {
    return v40;
}
const v53 = new BigUint64Array(BigUint64Array, BigUint64Array);
v53.copyWithin();
const v57 = RegExp.bind(2);
Math.fround();
Math.min(0.2148301603058278);
let v63 = 1073741824n;
v63 &= v63;
try { Float32Array(); } catch (e) {}
(-12n).toString();
([2036046200,-9223372036854775807,-61646])["map"](Array);
new Float64Array(16);
class C77 {
}
const v79 = new v57(1);
const v82 = new Int8Array(107);
for (let i85 = 0, i86 = 10; i85 < i86;) {
    v82[v79] = i85;
    ++i85;
    Math.max(-7);
}
new Int16Array(26);
4 == [4,7,4294967297,-1,1000];
const v102 = new Int8Array(Int8Array, Int8Array, Int8Array);
v102.includes(v102, v102);
new Uint8ClampedArray(6);
try {
    new WeakSet(4087n);
} catch(e109) {
} finally {
}
