const v1 = new WeakSet();
function F2(a4) {
    if (!new.target) { throw 'must be called with new'; }
    this.c = a4;
    this.b = a4;
}
const v5 = new F2(WeakSet);
new F2(F2);
function F7(a9, a10, a11) {
    if (!new.target) { throw 'must be called with new'; }
    const v13 = new Date();
    Reflect.get(v13.toUTCString);
}
new F2(v5);
const v21 = [-36998n];
const v22 = [-36998n,v1,v21,v21];
[1073741825n,v22,F2,v22];
const v29 = new Float32Array(3701);
const v32 = new Int32Array();
const v33 = new Int32Array(v32, 4096, v32);
function F34(a36) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v33;
    this.e = v32;
}
new Int32Array(6);
new Float32Array(0);
[-Infinity];
const o44 = {
    [4294967296]: 3701,
    "b": Float32Array,
    "e": v29,
    [1073741825n]: F34,
    "f": 6,
    set a(a43) {
    },
};
for (let v45 = 0; v45 < 5; v45++) {
    o44.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v49 = [13,9,-12559,-8,-4294967297];
let v50 = [65535,-1139741600];
new Uint8Array(v49);
let v53 = -41990;
const v54 = v50++;
let v55 = --v53;
function f56() {
    const v59 = new BigUint64Array(8);
    const v61 = new Uint32Array();
    for (const v62 of v59) {
        for (let v63 = 0; v63 < 32; v63++) {
            v61["p" + v63] = v63;
        }
    }
    return f56;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v55 * (v54 << v53);
