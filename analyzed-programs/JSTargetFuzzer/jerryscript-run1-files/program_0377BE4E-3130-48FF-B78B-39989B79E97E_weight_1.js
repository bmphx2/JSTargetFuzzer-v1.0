const v3 = [-1.0,-1.0,-1.0,-542041.3520148026];
const v4 = [-1.0,-542041.3520148026,v3];
const v5 = [-1.0,v3,v4];
function f6(a7, a8, a9, a10) {
    const o16 = {
        "c": -1.0,
        [a7](a12, a13, a14) {
            g = a14;
            return super.a;
        },
        "a": v5,
        ...a8,
        "d": -1.7679747380392775e+308,
        [v5]: a8,
        __proto__: v3,
        "h": -542041.3520148026,
        [a7]: -1.0,
    };
    return o16;
}
const v17 = f6(f6, v5, v4, v4);
f6(f6(v4, v4, v17, v3), v4, v4, v3);
class C20 {
    [v17] = -1.7679747380392775e+308;
    [f6] = f6;
    static get d() {
        let v22 = 0;
        do {
            Object.defineProperty(v5, 6, { writable: true, configurable: true, enumerable: true, get: f6 });
            try { v5.find(f6, v3); } catch (e) {}
            v22++;
        } while (v22 < 0)
        return C20;
    }
}
new C20();
new C20();
new C20();
new Float32Array(3701);
const v35 = new Int32Array();
const v36 = new Int32Array();
function F37(a39) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v36;
    this.e = v35;
}
new Int32Array(6);
new Float32Array(0);
[-Infinity];
const o47 = {
    set a(a46) {
    },
};
for (let v48 = 0; v48 < 5; v48++) {
    o47.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v52 = [13,9,-12559,-8,-4294967297];
let v53 = [65535,-1139741600];
new Uint8Array(v52);
let v56 = -41990;
const v57 = v53++;
let v58 = --v56;
function f59() {
    return f59;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v58 * (v57 << v56);
