new Uint8Array(255);
const v5 = new BigInt64Array(2);
const v8 = new BigUint64Array(6);
const v9 = [4.0,159.52717177809336,5.0,4.0,880.2425195192172];
const v10 = [-1000000000.0,591823.0901420095];
const v11 = [-437.48927047524285,-0.9662266889675308,4.0,-1000000000000.0,-1.0];
function f12(a13, a14, a15) {
    const o23 = {
        valueOf(a17, a18, a19) {
            a18++;
            try {
                super.compile(BigInt64Array);
            } catch(e22) {
            }
            return this;
        },
        "e": v8,
        "d": v10,
        "h": a15,
        "a": v5,
        __proto__: a14,
    };
    return a15;
}
f12(2, v11, v9);
f12(2, 2, v10);
f12(6, v8, v11);
new Float32Array(3701);
const v32 = new Int32Array();
const v33 = new Int32Array();
function F34(a36) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v33;
    this.e = v32;
}
new Int32Array(6);
new Float32Array(0);
[-Infinity];
const o44 = {
    set a(a43) {
    },
};
for (let v45 = 0; v45 < 5; v45++) {
    v45.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v49 = [13,9,-12559,-8,-4294967297];
let v50 = [65535,-1139741600];
new Uint8Array(v49);
let v53 = -41990;
const v54 = v50++;
let v55 = --v53;
function f56() {
    return f56;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v55 * (v54 << v53);
