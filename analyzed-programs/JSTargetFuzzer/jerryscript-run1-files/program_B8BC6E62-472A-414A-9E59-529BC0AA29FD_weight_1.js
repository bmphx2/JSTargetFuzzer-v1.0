let v1 = new Set();
let v3 = 0;
while (v3 < 2) {
    (1248 + v3) + Set;
    let {"b":v8,"d":v9,"size":v10,} = v1;
    v3++;
}
const v13 = new Uint8Array(1248);
const v16 = new Uint8Array(256);
v16[-2] &= 1248;
let v17 = WeakMap;
new v17();
let v22 = 13;
new Uint16Array(v22);
let v25 = Date;
new v25();
new Float32Array(3701);
v13[7] = 6;
v22 = 256;
const v35 = new Int32Array();
const v36 = new Int32Array();
function F37(a39) {
    if (!new.target) { throw 'must be called with new'; }
    this.f = v36;
    this.e = v35;
}
new Int32Array(6);
new Float32Array(0);
[-Infinity];
const o47 = {
    set a(a46) {
    },
};
for (let v48 = 0; v48 < 5; v48++) {
    o47.a;
}
[-6.568975937209441,1.0,-Infinity];
[0.6730197545704021,2.220446049250313e-16,5.387745681974783,-7.304740854736329,89425.84876466752,1000000000.0,-1.785661314585028e+308];
const v52 = [13,9,-12559,-8,-4294967297];
let v53 = [65535,-1139741600];
const v55 = new Uint8Array(v52);
v55.valueOf = v25;
("nYmP").length;
let v60 = -41990;
const v61 = v53++;
let v62 = --v60;
function f63() {
    ({"f":v1,"h":v17,"length":v25,} = "undefined");
    const v64 = [48392,-19641,-8,4294967297];
    [448026296,-1623527531,-9007199254740990,-2119923303,1000,-40822];
    const v66 = [-6,26222];
    function f69(a70, a71) {
        try { a71(v64, Math, v66); } catch (e) {}
        return v64;
    }
    f69(v35, Uint8Array);
    Math.hypot(1000.0);
    Math.round(-1067404640);
    v52 + 1000.0;
    Math.hypot(1000.0);
    const t55 = "a";
    t55.__proto__ = v55;
    return f63;
}
([-8.370930286710454,7.183159495332482,2.2250738585072014e-308,-1000000000000.0,-255.6362008927648,-315.07126899047114,-1.4473359933311645e+308,0.776153163138491,787700.9653515914]).flatMap(TypeError);
--v62;
6 * (v61 << v60);
